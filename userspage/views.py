...
def index(request):
    ...
    return render(request, 'userpage/index.html',contex)
...
# def to show product list in user page
...
def user_login(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if user is not None:#if uesr xa vane
                login(request,user)
                if user.is_staff:
...
# def to add products to the cart
@login_required
def add_to_cart(request,product_id):
    user=request.user
    product=Product.objects.get(id=product_id)
    check_item_present=Cart.objects.filter(product=product , user=user)
    if check_item_present:
        ...
        if cart:
           ...
           return redirect('/cart')
            ...
            messages.add_message(request, messages.ERROR,'Product failed to add to cart')
            return redirect('/cart')
